...
export function CreatorRadio() {
  ...
  return <>
    ...
    <button className="creator-radio__support" type="button" onClick={() => setSupportOpen(true)}>
      <Heart size={10} /> Keep the memories playing
    </button>
    ...
    <AnimatePresence>
      {supportOpen && <motion.div className="support-overlay" role="presentation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => {
        ...
        <motion.section className="support-sheet" role="dialog" aria-modal="true" aria-labelledby="support-title" initial={{ opacity: 0, y: 24, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 16, scale: .98 }} transition={{ duration: .3, ease: [0.22, 1, 0.36, 1] }}>
          ...
          <p className="support-sheet__eyebrow"><span /> A little love</p>
